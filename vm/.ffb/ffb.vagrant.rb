...
class FfbVagrant
  ...
  def FfbVagrant.run
    ...
    vagrant_command = nil
    ARGV.each { |cmd_arg| ['box', 'connect', 'destroy', 'global-status', 'halt', 'init', 'login', 'package', 'plugin', 'port', 'powershell', 'provision', 'rdp', 'reload', 'resume', 'share', 'snapshot', 'ssh', 'ssh-config', 'status', 'suspend', 'up', 'validate', 'version'].include?(cmd_arg) == true ? vagrant_command = cmd_arg : vagrant_command = 'unknown'; }
    ...
    Tools::User.setup(LOCAL_USER_SETTINGS_DIR, LOCAL_CONFIG_FILENAME)
